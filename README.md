...
- **カテゴリ**: エントリをフォルダに整理。未分類は即表示、カテゴリ入りは開くまで非表示（起動時は全て閉じた状態）
- **コピーの自動失効**: エントリのコピーは60秒後にクリップボードから自動消去（設定でOFF可）
- **Spotlight連携**: iOSのSpotlight検索からエントリ名で直接ジャンプ（エントリ名のみ登録・内容は非公開、EASビルドで有効）
